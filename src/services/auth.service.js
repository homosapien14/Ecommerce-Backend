...
const loginUserWithEmailAndPassword = async (email, password) => {
  ...
  if (!user) throw new ApiError(httpStatus.UNAUTHORIZED, `User with ${email} not found`);
  const isPasswordMatched = await user.isPasswordMatch(password);
  if (!isPasswordMatched) throw new ApiError(httpStatus.UNAUTHORIZED, `Invalid credentials`);
  return user;
};
