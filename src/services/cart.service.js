...
const getCartByUser = async (user) => {
  ...
  if (!userCart) throw new ApiError(httpStatus.NOT_FOUND, "User does not have a cart");
  // if (userCart.cartItems.length == 0) throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, "User cart is empty");
  return userCart;
};
...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  if (!product) throw new ApiError(httpStatus.BAD_REQUEST, "Product doesn't exist in database");
    ...
    const userCart = await Cart.findOne({ email: user.email });
    if (!userCart) throw new ApiError(httpStatus.BAD_REQUEST, "User does not have a cart. Use POST to create cart and add a product");
    else if (userCart) {
...
const deleteProductFromCart = async (user, productId) => {
  const userCart = await Cart.findOne({ email: user.email });
  if (!userCart) throw new ApiError(httpStatus.BAD_REQUEST, "User does not have a cart");
  else if (userCart) {
...
const checkout = async (user) => {
  ...
  if (userCart.cartItems.length == 0) throw new ApiError(httpStatus.BAD_REQUEST, `User does not have any products added in the cart`);
  if (!(await user.hasSetNonDefaultAddress())) throw new ApiError(httpStatus.BAD_REQUEST, `Set User address first`);
  let totalCartValue = 0;
  for (let item of userCart.cartItems) {
    totalCartValue += item.product.cost * item.quantity;
  }
  if (user.walletMoney < totalCartValue) throw new ApiError(httpStatus.BAD_REQUEST, `Insufficient funds in user's wallet`)
